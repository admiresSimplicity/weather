...
const API_KEY = "761aa595a85044a1afe81936241409";
...
const displayHourlyForecast = (hourlyData) => {
  ...
  // Filter the hourly datya only include the next 24 hours.
...
const getWeatherDetails = async (API_URL) => {
  ...
  try {
    // Api dan verileri cektik JSOn araciligi ile response ettik.
    const response = await fetch(API_URL);
    const data = await response.json();

    // Anlik olan bilgileri degiskenlere atadik
    ...
    //Update current weather on display
...
locationButton.addEventListener("click", () => {
  navigator.geolocation.getCurrentPosition(
    ...
    (error) => {
      alert("Location erisim reddedildi.");
    }
  );
});
